...
# %%
fp_point = fixed_point(se, sp)
if p_hat < fp_point:
    prediction = "raw rate OVERSTATES the true rate — correction should move it DOWN"
elif p_hat > fp_point:
    prediction = "raw rate UNDERSTATES the true rate — correction should move it UP"
else:
    prediction = "raw rate sits at the fixed point — expect almost no correction"
...
theta_hat = corrected_rate(p_hat, se, sp)
theta_hat_display = min(max(theta_hat, 0.0), 1.0)
