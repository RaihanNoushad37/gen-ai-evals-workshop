...
cm_v1 = confusion(clean_v1["label"].tolist(), clean_v1["verdict"].tolist())
r_v1 = rates(cm_v1)
...
# %%
se_ci = wilson_interval(cm_v1["tp"], cm_v1["tp"] + cm_v1["fn"])
sp_ci = wilson_interval(cm_v1["tn"], cm_v1["tn"] + cm_v1["fp"])
...
for class_label, metric_name in [(1, "sensitivity"), (0, "specificity")]:
    ...
    comparison = paired_compare(correct_v1, correct_v2)
    discordance = (comparison["n01"] + comparison["n10"]) / len(subset)
    mde = minimum_detectable_effect(len(subset), discordance)
