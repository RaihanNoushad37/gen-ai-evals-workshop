...
from evals_workshop.metrics import confusion, rates
...
examples = select_few_shot(disagreements, n=2)

if examples.empty:
    print("v1 got every dev row right — nothing to learn from. Keeping v1 as v2.")
    RUBRIC_V2 = RUBRIC_V1
else:
    RUBRIC_V2 = add_few_shot_examples(RUBRIC_V1, examples)
...
cm = confusion(clean["label"].tolist(), clean["verdict"].tolist())
r = rates(cm)
